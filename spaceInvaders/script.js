...
function gameLoop(){
  ...
  if(lives <= 0 && score <= 44 || !gameRunning){
    gameRunning=false;
  ...
  if(aliveInvaders == 0){
    gameRunning=false;
    drawGameOver("Ganaste :D");
    drawBottomHelper();
    alert('Has ganado una coca :DD');
    return false;
  }
  ctx.clearRect(0,0,canvas.width,canvas.height);
  helperHandler();
...
function helperHandler(){
  if(aliveInvaders == armyColumns* armyRows){
    drawBottomMessage("presiona X para disparar", 125);
  }  else
  if(hasLifeDecreased){
    drawBottomMessage(`Vidas restantes: ${lives}`, 150);
    setTimeout(() => {
    hasLifeDecreased=false;
    ...
    for (let i = 0; i < armyRows; i++) {
      for(let j = 0; j < armyColumns; j++){
          ...
          )
          {
            ...
            //increase speed
            if((aliveInvaders)%armyColumns==0 && armySpeed > 7){
...
function drawBlinker(func1, func2){
  ...
  function drawStartScreen(){
    drawBlinker(function(){ drawScreen__line1("Space Invaders") },function(){ drawScreen__line2("Presiona Enter para jugar") });
  }
